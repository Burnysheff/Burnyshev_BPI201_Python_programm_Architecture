...
class diagonalMatrix(Matrix):
    ...
    def Enter(self, arraySymbols, position):
        ...
        for i in range (self.size):
            if position < len(arraySymbols):
                self.element[i] = int(arraySymbols[position])
                position += 1
            else:
                self.element[i] = 1
        return position
    ...
    def EnterRandom(self):
        ...
        for i in range (self.size):
            self.element.append(0)
        for i in range (self.size):
            self.element[i] = random.randint(1, 100)

    def Print(self, outfile):
        outfile.write("This is a diagonal matrix!")
        outfile.write("\n")
        for i in range(self.size):
            for j in range(self.size):
                if i == j:
                    outfile.write(str(self.element[i]))
                    outfile.write(" ")
                else:
                    outfile.write("0 ")
            outfile.write("\n")
